-- =====================================================
-- Migration: Populate Configuration Tables
-- =====================================================
-- Description: Migrates hardcoded data from data.ts to database
-- Hierarchy: global_config → power_config → org_config
-- Date: 2026-01-23
-- =====================================================

-- =====================================================
-- PART 1: GLOBAL CONFIG
-- Regras federais que se aplicam a todos os órgãos
-- =====================================================
...
-- 1.2. Tabelas de PSS (Previdência Social do Servidor)
INSERT INTO global_config (config_key, config_value, valid_from, valid_to)
VALUES (
  'pss_tables',
  '{
...
-- 1.3. Tabelas de IR (Imposto de Renda)
INSERT INTO global_config (config_key, config_value, valid_from, valid_to)
VALUES (
  'ir_deduction',
  '{
...
-- =====================================================
-- PART 2: POWER CONFIG (PJU - Poder Judiciário da União)
-- Regras compartilhadas por todos os órgãos do PJU
-- =====================================================

-- 2.1. CJ1 Integral Base (para cálculo de VR)
...
-- 2.2. Bases Salariais do PJU (Analistas e Técnicos)
INSERT INTO power_config (power_name, config_key, config_value, valid_from, valid_to)
VALUES (
  'PJU',
  'salary_bases',
...
-- 2.3. Multiplicadores de AQ (Adicional de Qualificação - Lei 15.292/2025)
INSERT INTO power_config (power_name, config_key, config_value, valid_from, valid_to)
VALUES (
  'PJU',
  'aq_rules',
...
-- 2.4. Percentuais de Gratificações do PJU
INSERT INTO power_config (power_name, config_key, config_value, valid_from, valid_to)
VALUES (
  'PJU',
...
-- Verify global_config
-- SELECT config_key, valid_from, valid_to 
-- FROM global_config 
-- WHERE valid_to IS NULL
-- ORDER BY config_key;

-- Verify power_config for PJU
-- SELECT config_key, valid_from, valid_to 
-- FROM power_config 
-- WHERE power_name = 'PJU' AND valid_to IS NULL
-- ORDER BY config_key;

-- Test ConfigService merge (requires application code)
-- SELECT * FROM org_config WHERE org_slug = 'jmu';
...
-- 3. Test ConfigService.getEffectiveConfig('jmu')
-- 4. Deprecate data.ts file
-- =====================================================
